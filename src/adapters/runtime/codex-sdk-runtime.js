...
function buildPrompt(invocation) {
  ...
  if (invocation.operation === "planning") {
    operationInstruction = [
          ...
          "A selected repository_check is authoritative only when ChangeFleet runs it after publishing the exact Candidate commit in an otherwise clean workspace where HEAD is that Candidate and work_unit.base_sha is the exact base. Select argv that exercises the intended base-to-Candidate subject in that state; a command whose default behavior inspects only uncommitted changes does not cover the Candidate diff.",
